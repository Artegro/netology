...
<details>  
   ...
   Создаем [Манифест](./s3baket/vm.tf_) с 3 ся виртуалками, поместим его в папку с остальными манифестами, для этого возьмем манифест из прошлых домашек и слегка переделаем.  
...
*  **Работа над ошибками**
   ...
   *  `5 Кроме того, я бы вынес и параметры ВМ (ЦПУ, память, версию ОС и пр) в variables`
      *  Ответ:  
      Изначально даже не подумал о таком варианте, спасибо это можут быть полезно, пока моим девизом было чем проще тем лучше ну и давящие сроки.
      В дальнейшем мы будем бить виртуальные машины на группы и для каждой группы, и по группам далее будет различия по параметрам как минимум памяти, так что в нашем случае не сильно актуально на текущий момент.
